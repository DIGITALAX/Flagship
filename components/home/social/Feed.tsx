  ...
  return (
    <div className="relative w-full h-full flex">
      <InfiniteScroll
        ...
        hasMore={true}
        loader={""}
        className="relative w-full h-fit gap-6 grid grid-flow-row auto-rows-auto p-1"
      >
        {feedLoading
          ? Array.from({ length: 20 })?.map((_, index: number) => {
              return (
                <div
                  className={`relative w-full animate-pulse opacity-70 h-[10rem] flex flex-row flex-wrap sm:flex-nowrap gap-6 rounded-md z-0`}
                  key={index}
                >
                  <div
